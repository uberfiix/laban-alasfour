...
export function RoomExperience({ modelUrl, initialColors, className = "" }: RoomExperienceProps) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl border border-secondary/30 bg-gradient-to-br from-muted/40 to-card ${className}`}>
      <Canvas
        shadows
      ...
      {/* Color picker */}
      <div className="pointer-events-auto absolute right-3 top-3 max-w-[230px] rounded-2xl border border-border/60 bg-card/90 p-3 shadow-soft backdrop-blur-md" dir="rtl">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          <button
            ...
            title="اللون الأصلي"
            className={`h-7 w-7 rounded-full border-2 bg-gradient-to-br from-muted to-background ${
      ...
      {/* Bottom controls */}
      <div className="pointer-events-auto absolute bottom-3 left-3 flex flex-wrap items-center gap-2" dir="rtl">
        <div className="flex items-center gap-2 rounded-xl border border-border/60 bg-card/90 px-3 py-1.5 text-xs font-medium text-foreground shadow-soft backdrop-blur-md">
          <Move3D className="h-3.5 w-3.5 text-secondary" />
          اسحب المقابض الملوّنة لتحريك القطعة داخل الغرفة
        </div>
        <Button
          size="sm"
          variant="outline"
      ...
      <div className="absolute right-3 bottom-3 rounded-full bg-secondary px-3 py-1.5 text-xs font-semibold text-secondary-foreground shadow-gold">
        تجربة VR تفاعلية
      </div>
    </div>
  );
